...
class objective:
    ...
    def rf(self, trial): 
        ...
        clf=RandomForestClassifier(**space, bootstrap = False, random_state = 123, n_jobs = -1)
        X1, X2, y1, y2 = train_test_split(self.X, self.y, test_size = 0.2, random_state=123)
        clf.fit(X1, y1)
        preds = clf.predict(X2)
        evalu = f1_score(y2, preds, average = "binary")
...
def stack_tuning(X, y, models):
    """
    Function used for hyperparameter tuning the stacked models through hyperparameter_base.py
    """
    # init dicts
    study_dict = {}
    best_params = {}
    for mod in models:
        stack_tuner = objective(X[mod], y) #define objective for each model
        np.random.seed(42)
